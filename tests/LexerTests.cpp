...
TEST_CASE("While token", "[Lexer tests]") {
    ...
    CHECK(token->getType() == TokenType::T_WHILE);
}
...
TEST_CASE("If token", "[Lexer tests]") {
    ...
    Lexer lexer(source);
    auto token = lexer.getNextToken();
    CHECK(token->getType() == TokenType::T_IF);
}
...
TEST_CASE("Elsif token", "[Lexer tests]") {
    ...
    Lexer lexer(source);
    auto token = lexer.getNextToken();
    CHECK(token->getType() == TokenType::T_ELSIF);
}
...
TEST_CASE("Else token", "[Lexer tests]") {
    ...
    Lexer lexer(source);
    auto token = lexer.getNextToken();
    CHECK(token->getType() == TokenType::T_ELSE);
}
...
TEST_CASE("Return token", "[Lexer tests]") {
    ...
    Lexer lexer(source);
    auto token = lexer.getNextToken();
    CHECK(token->getType() == TokenType::T_RETURN);
}
...
TEST_CASE("Assign token", "[Lexer tests]") {
    ...
    Lexer lexer(source);
    auto token = lexer.getNextToken();
    CHECK(token->getType() == TokenType::T_ASSIGN);
}
...
TEST_CASE("Open token", "[Lexer tests]") {
    ...
    Lexer lexer(source);
    auto token = lexer.getNextToken();
    CHECK(token->getType() == TokenType::T_OPEN);
}
...
TEST_CASE("Close token", "[Lexer tests]") {
    ...
    Lexer lexer(source);
    auto token = lexer.getNextToken();
    CHECK(token->getType() == TokenType::T_CLOSE);
}
...
TEST_CASE("OpenBracket token", "[Lexer tests]") {
    ...
    Lexer lexer(source);
    auto token = lexer.getNextToken();
    CHECK(token->getType() == TokenType::T_OPEN_BRACKET);
}
...
TEST_CASE("CloseBracket token", "[Lexer tests]") {
    ...
    Lexer lexer(source);
    auto token = lexer.getNextToken();
    CHECK(token->getType() == TokenType::T_CLOSE_BRACKET);
}
...
TEST_CASE("Semicolon token", "[Lexer tests]") {
    ...
    Lexer lexer(source);
    auto token = lexer.getNextToken();
    CHECK(token->getType() == TokenType::T_SEMICOLON);
}
...
TEST_CASE("Comma token", "[Lexer tests]") {
    ...
    Lexer lexer(source);
    auto token = lexer.getNextToken();
    CHECK(token->getType() == TokenType::T_COMMA);
}
...
TEST_CASE("Less token", "[Lexer tests]") {
    ...
    Lexer lexer(source);
    auto token = lexer.getNextToken();
    CHECK(token->getType() == TokenType::T_LESS);
}
...
TEST_CASE("Greater token", "[Lexer tests]") {
    ...
    Lexer lexer(source);
    auto token = lexer.getNextToken();
    CHECK(token->getType() == TokenType::T_GREATER);
}
...
TEST_CASE("LessOrEqual token", "[Lexer tests]") {
    ...
    Lexer lexer(source);
    auto token = lexer.getNextToken();
    CHECK(token->getType() == TokenType::T_LESS_OR_EQUAL);
}
...
TEST_CASE("GreaterOrEqual token", "[Lexer tests]") {
    ...
    Lexer lexer(source);
    auto token = lexer.getNextToken();
    CHECK(token->getType() == TokenType::T_GREATER_OR_EQUAL);
}
...
TEST_CASE("Equal token", "[Lexer tests]") {
    ...
    Lexer lexer(source);
    auto token = lexer.getNextToken();
    CHECK(token->getType() == TokenType::T_EQUAL);
}
...
TEST_CASE("NotEqual token", "[Lexer tests]") {
    ...
    Lexer lexer(source);
    auto token = lexer.getNextToken();
    CHECK(token->getType() == TokenType::T_NOT_EQUAL);
}
...
TEST_CASE("Plus token", "[Lexer tests]") {
    ...
    Lexer lexer(source);
    auto token = lexer.getNextToken();
    CHECK(token->getType() == TokenType::T_PLUS);
}
...
TEST_CASE("Minus token", "[Lexer tests]") {
    ...
    Lexer lexer(source);
    auto token = lexer.getNextToken();
    CHECK(token->getType() == TokenType::T_MINUS);
}
...
TEST_CASE("Or token", "[Lexer tests]") {
    ...
    Lexer lexer(source);
    auto token = lexer.getNextToken();
    CHECK(token->getType() == TokenType::T_OR);
}
...
TEST_CASE("Mul token", "[Lexer tests]") {
    ...
    Lexer lexer(source);
    auto token = lexer.getNextToken();
    CHECK(token->getType() == TokenType::T_MUL);
}
...
TEST_CASE("Div token", "[Lexer tests]") {
    ...
    Lexer lexer(source);
    auto token = lexer.getNextToken();
    CHECK(token->getType() == TokenType::T_DIV);
}
...
TEST_CASE("And token", "[Lexer tests]") {
    ...
    Lexer lexer(source);
    auto token = lexer.getNextToken();
    CHECK(token->getType() == TokenType::T_AND);
}
...
TEST_CASE("TypeVoid token", "[Lexer tests]") {
    ...
    Lexer lexer(source);
    auto token = lexer.getNextToken();
    CHECK(token->getType() == TokenType::T_TYPE_VOID);
}
...
TEST_CASE("TypeInt token", "[Lexer tests]") {
    ...
    Lexer lexer(source);
    auto token = lexer.getNextToken();
    CHECK(token->getType() == TokenType::T_TYPE_INT);
}
...
TEST_CASE("TypeFloat token", "[Lexer tests]") {
    ...
    Lexer lexer(source);
    auto token = lexer.getNextToken();
    CHECK(token->getType() == TokenType::T_TYPE_FLOAT);
}
...
TEST_CASE("TypeString token", "[Lexer tests]") {
    ...
    Lexer lexer(source);
    auto token = lexer.getNextToken();
    CHECK(token->getType() == TokenType::T_TYPE_STRING);
}
...
TEST_CASE("TypeGeo token", "[Lexer tests]") {
    ...
    Lexer lexer(source);
    auto token = lexer.getNextToken();
    CHECK(token->getType() == TokenType::T_TYPE_GEO);
}
...
TEST_CASE("TypeGeocoord token", "[Lexer tests]") {
    ...
    Lexer lexer(source);
    auto token = lexer.getNextToken();
    CHECK(token->getType() == TokenType::T_TYPE_GEOCOORD);
}

TEST_CASE("Int token", "[Lexer tests]") {
    StringSource source("5");
    Lexer lexer(source);
    auto token = lexer.getNextToken();
...
TEST_CASE("String token", "[Lexer tests]") {
    SECTION("Empty string") {
        StringSource source("\"\"");
        Lexer lexer(source);
        CHECK(lexer.getNextToken()->getType() == TokenType::T_STRING);
    }
...
TEST_CASE("GeoDegree token", "[Lexer tests]") {
    ...
    Lexer lexer(source);
    auto token = lexer.getNextToken();
    CHECK(token->getType() == TokenType::T_GEO_DEGREE);
}
...
TEST_CASE("GeoMinute token", "[Lexer tests]") {
    ...
    Lexer lexer(source);
    auto token = lexer.getNextToken();
    CHECK(token->getType() == TokenType::T_GEO_MINUTE);
}
...
TEST_CASE("GeoSecond token", "[Lexer tests]") {
    ...
    Lexer lexer(source);
    auto token = lexer.getNextToken();
    CHECK(token->getType() == TokenType::T_GEO_SECOND);
}
...
TEST_CASE("GeoDirectionN token", "[Lexer tests]") {
    ...
    Lexer lexer(source);
    auto token = lexer.getNextToken();
    CHECK(token->getType() == TokenType::T_GEO_DIRECTION_N);
}
...
TEST_CASE("GeoDirectionS token", "[Lexer tests]") {
    ...
    Lexer lexer(source);
    auto token = lexer.getNextToken();
    CHECK(token->getType() == TokenType::T_GEO_DIRECTION_S);
}
...
TEST_CASE("GeoDirectionW token", "[Lexer tests]") {
    ...
    Lexer lexer(source);
    auto token = lexer.getNextToken();
    CHECK(token->getType() == TokenType::T_GEO_DIRECTION_W);
}
...
TEST_CASE("GeoDirectionE token", "[Lexer tests]") {
    ...
    Lexer lexer(source);
    auto token = lexer.getNextToken();
    CHECK(token->getType() == TokenType::T_GEO_DIRECTION_E);
}
...
TEST_CASE("Multicomment token", "[Lexer tests]") {
    ...
    Lexer lexer(source);
    auto token = lexer.getNextToken();
    CHECK(token->getType() == TokenType::T_MULTICOMMENT);
}
...
TEST_CASE("End token", "[Lexer tests]") {
    ...
    Lexer lexer(source);
    auto token = lexer.getNextToken();
    CHECK(token->getType() == TokenType::T_END);
}
...
TEST_CASE("Unknown token", "[Lexer tests]") {
    ...
    Lexer lexer(source);
    auto token = lexer.getNextToken();
    //CHECK(token->getType() == TokenType::T_UNKNOWN);
    //CHECK(token->getType() == TokenType::T_UNKNOWN);
    //CHECK_THROWS(lexer.getNextToken());
    CHECK_THROWS_AS(lexer.getNextToken(), LexerException);
}